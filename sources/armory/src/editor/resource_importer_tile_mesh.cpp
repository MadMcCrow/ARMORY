...
/**
 *  this is stolen from godot : editor/import/resource_importer_obj.cpp
 */
 ...
 *  this is stolen from godot : editor/import/resource_importer_obj.cpp
 */
static Error _parse_obj(const String &p_path, List<Ref<TileMesh>> &r_meshes, bool p_single_mesh, bool p_generate_tangents, bool p_optimize, int p_tile_type, Vector3 p_scale_mesh, Vector3 p_offset_mesh, List<String> *r_missing_deps) {
	FileAccessRef f = FileAccess::open(p_path, FileAccess::READ);
	...
	while (true) {
		...
		if (l.begins_with("v ")) {
			...
			if (surf_tool->get_vertex_array().size()) {
				...
				tile = surf_tool->commit(tile, mesh_flags);

				if (current_material != String()) {
					tile->surface_set_name(tile->get_surface_count() - 1, current_material.get_basename());
				} else if (current_group != String()) {
					tile->surface_set_name(tile->get_surface_count() - 1, current_group);
				}

				print_verbose("OBJ: Added surface :" + tile->surface_get_name(tile->get_surface_count() - 1));
				surf_tool->clear();
				surf_tool->begin(Mesh::PRIMITIVE_TRIANGLES);
			}
	...
	if (p_single_mesh) {
		tile->set_geometry(p_tile_type); // set tile type here
		r_meshes.push_back(tile);
