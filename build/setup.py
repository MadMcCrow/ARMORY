...
def generateDesktopFile(path):
	...
	try:
		...
		bin_path = findFilesList(GodotPath + "/bin", [str(ProjectName).lower(), ".tools.", "bin"])[0]
	...
	try:
		# \see import config
		arguments= "${path_to_project} --editor --upwards".format(path_to_project = project_path)
		d= text.format(Name = ProjectName, Comment = Description, bin = bin_path, args = arguments, icon = icon_path, bterm = "true")
...
#
def build():
	buildcommand= "scons -j{Threads}  profile={Custom}"
	try:
		import multiprocessing
		cpu= multiprocessing.cpu_count()
		# \see import config
		cmd= buildcommand.format(Threads = cpu, Custom = CustomPath)
		import subprocess
		print("building godot with command : \n{}".format(cmd))
		rc= subprocess.call(cmd, cwd = GodotPath, shell = True)
	except:
		print("Failed to buid {} with {}".format(GodotPath, CustomPath))
		raise
